...
const BaseStrategyPrototype = {
  /**
   * [getLastCommittedVersion description]
   * @param  {[type]} aggregateId [description]
   * @return {[type]}             [description]
   */
  getLastCommittedVersion: function (aggregateId) {
    const events = this.getEvents(aggregateId, {limit: 1, order: 'desc'});
    return (events.length) ? events[0].version : 0;
  },
  ...
  */
  ...
  validateStrategy: function() {
    const missing = requiredMethods.filter(opt => typeof this[opt] !== 'function');
    if (missing.length) {
      const err = new Error(`Missing functions from eventStore definition: ${missing.join(', ')}`);
      throw err;
    }
  },
...
export default BaseStrategyPrototype;
